...
def get_networking(update: Updater, context: CallbackContext):
    ...
    if not context.bot_data["user"].active:
        keyboard = [
            [InlineKeyboardButton("Подтвердить участие", callback_data="confirm")],
            [InlineKeyboardButton("Главное меню", callback_data="to_start")],
        ]
        context.bot.send_message(
            chat_id=update.effective_chat.id,
...
def get_notifications(update: Updater, context: CallbackContext):
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="Вы подписались на нашу рассылку! Мы уведомим вас о датах следующих мероприятий!",
    )
